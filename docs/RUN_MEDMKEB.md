...
ENGRAM is available through the same runner:
...
python scripts/medmkeb/run_medmkeb_editing.py \
  ...
  --max-edits 5 \
  --output-dir <OUTPUT_DIR>/engram_run
```

Adapter-only dry-run, with no model loading:
...
python scripts/medmkeb/run_medmkeb_editing.py \
  ...
  --dry-run \
  --max-edits 2 \
  --output-dir <OUTPUT_DIR>/engram_dry_run
```

Use `hparams/ENGRAM/llava_med.yaml` for LLaVA-Med after replacing
`<MODEL_PATH>`, `<VISION_TOWER_PATH>`, and `<IMAGE_ROOT>`. The LLaVA-Med dataset
path keeps image paths in records because the wrapper handles official image
preprocessing internally.

ENGRAM builds `X_plus` from edit/rephrase/image-rephrase variants and `X_minus`
from text locality, multimodal locality, and optional retain-pool variants. If
no reference variants are available, the editor logs a warning because the
estimate becomes target-only erasure.

Bank utilities:

```bash
python scripts/engram/list_engram_bank.py --bank <BANK_DIR> --export-csv <OUTPUT_DIR>/engram_bank.csv
python scripts/engram/compute_engram_overlap.py --bank <BANK_DIR> --out <OUTPUT_DIR>/engram_overlap_report
python scripts/engram/apply_engram_bank.py --bank <BANK_DIR> --edit-id <EDIT_ID> --dry-run
python scripts/engram/rollback_engram.py --bank <BANK_DIR> --edit-id <EDIT_ID> --dry-run
```

Mock alpha/layer experiments, with no model weights:
...
Replacement mode is implemented only as an experimental path using explicit
state-dict pairs. It should not be reported as a solved medical replacement
editing method without real-model validation.
