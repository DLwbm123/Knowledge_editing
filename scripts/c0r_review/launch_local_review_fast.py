...
function freeze(){{if(!valid())return;if(['4','5'].includes(preset.value)&&confirmStage===0){{confirmStage=1;confirmBar.style.display='block';return}}confirmed.value='true';form.querySelectorAll('button').forEach(x=>x.disabled=true);HTMLFormElement.prototype.submit.call(form)}}
