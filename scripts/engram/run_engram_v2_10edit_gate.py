#!/usr/bin/env python3
"""Frozen single-order ten-edit gate for SR-TR ENGRAM V2."""
...
def classify(checks: Mapping[str, bool], final_gains: Mapping[str, float], degradation: Mapping[str, float]) -> str:
    ...
    if not checks["bank"] or not checks["rollback"]:
        return "ENGRAM_V2_10EDIT_BANK_FAILURE"
    if not checks["solver"]:
        return "ENGRAM_V2_NUMERICAL_INSTABILITY"
    if all(checks.values()):
        return "ENGRAM_V2_10EDIT_PASS"
    ...
    return "ENGRAM_V2_10EDIT_PARTIAL"


def recommendation(gate: str) -> str:
    if gate == "ENGRAM_V2_10EDIT_PASS":
        return "PROMOTE_TO_MULTI_SEED_10EDIT"
    if gate in ("ENGRAM_V2_10EDIT_PARTIAL", "ENGRAM_V2_INTERFERENCE_ACCUMULATION", "ENGRAM_V2_NUMERICAL_INSTABILITY"):
...
def main() -> None:
    ...
    result_path = args.out_dir / "run" / "main_result.json"
    if result_path.exists():
        raise FileExistsError(f"Ten-edit main result exists: {result_path}")
    ...
    if bank_root.exists():
        raise FileExistsError(f"Ten-edit bank exists: {bank_root}")
    ...
    result = {
        ...
        "constraints": {
            ...
            "repeat_seed": False, "ten_edit": True, "twenty_edit": False,
            "physical_gpu": 0, "gpu2_used": False, "algorithm_or_config_modified": False,
        },
    }
    write_json(result_path, result)
    write_csv(args.out_dir / "ENGRAM_V2_10EDIT_STAGE_METRICS.csv", metric_rows, ["state", "record_id", "view", "baseline_nll", "current_nll", "improvement", "abs_drift", "success"])
    write_csv(args.out_dir / "ENGRAM_V2_10EDIT_RETENTION_MATRIX.csv", retention_rows, ["state", *ORDER])
    write_json(args.out_dir / "ENGRAM_V2_10EDIT_LOCALITY.json", locality)
    write_json(args.out_dir / "ENGRAM_V2_10EDIT_DELTA_ANALYSIS.json", delta)
    write_json(args.out_dir / "run/same_process_bank_equivalence.json", {"prefixes": bank_rows, "summary": bank_summary})
    write_json(args.out_dir / "ENGRAM_V2_10EDIT_ROLLBACK.json", rollback)
    print(json.dumps({
        "final_gate": gate, "fixed_sequence": ORDER,
        "edits_retained": f"{summary['edits_retained']}/10",
